...
安装
[uni-ui 组件库](https://uniapp.dcloud.net.cn/component/uniui/quickstart.html#npm安装)
...
说明：`Pinia` 用法与 `Vue3` 项目完全一致，`uni-app`
项目仅需解决**持久化插件兼容性**问题。

### 持久化存储插件

安装持久化存储插件：
[pinia-plugin-persistedstate](https://prazdevs.github.io/pinia-plugin-persistedstate/zh/guide/config.html#storage)
...
tip 温馨提示到此，你已完成 `eslint` + `prettier` 的配置。
...
tip 温馨提示到此，你已完成 `husky` + `lint-staged` 的配置。
